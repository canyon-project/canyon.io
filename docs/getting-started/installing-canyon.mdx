...
建表sql文件在 [canyon/packages/canyon-backend/prisma/schema.sql](https://github.com/canyon-project/canyon/blob/dev/packages/canyon-backend/prisma/create-table.sql) 中
...
2. `GITLAB_URL`: gitlab的URL。例如：`https://gitlab.com`.
3. `GITLAB_CLIENT_ID`: 从 Gitlab OAuth 配置中获取的客户端 ID。
4. `GITLAB_CLIENT_SECRET`: 从 Gitlab OAuth 配置中获取的客户端密钥。
5. `PRIVATE_TOKEN`: 申请一个私有令牌，以便在 Gitlab 上执行操作。
6. `REDIRECT_URI`: 重定向 URI 用于 OAuth。
